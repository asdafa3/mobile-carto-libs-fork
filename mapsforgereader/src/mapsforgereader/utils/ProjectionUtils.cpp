...
namespace carto {
    mvt::MapPos Projection::fromLatLon(double lat, double lng) const {
        return fromWgs84(MapPos(lng, lat));
    }

    mvt::MapPos Projection::toLatLon(double x, double y) const {
        MapPos wgs84Pos = toWgs84(MapPos(x, y));
        return MapPos(wgs84Pos(2), wgs84Pos(1));
    }

    mvt::MapPos EPSG3857::fromWgs84(const MapPos& wgs84Pos) const {
    ...
    mvt::MapPos EPSG3857::toWgs84(const MapPos& mapPos) const {
