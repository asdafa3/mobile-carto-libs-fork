...
namespace carto {
    MapPos AbstractProjection::fromLatLon(double lat, double lng) const {
        return fromWgs84(MapPos(lng, lat));
    }

    MapPos AbstractProjection::toLatLon(double x, double y) const {
        MapPos wgs84Pos = toWgs84(MapPos(x, y));
        return MapPos(wgs84Pos(2), wgs84Pos(1));
    }

    MapPos EPSG3857Projection::fromWgs84(const MapPos& wgs84Pos) const {
    ...
    MapPos EPSG3857Projection::toWgs84(const MapPos& mapPos) const {
