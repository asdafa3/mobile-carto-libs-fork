...
namespace carto {
    namespace mapsforge {
        ...
        bool MapFile::containsTile(const MapTile &tile) {
            ...
            MapBounds bboxTile3D = _tileTransformer->calculateTileBBox(tile);
            MapBounds bboxTile2D = Converter::bbox3tobbox2(bboxTile3D);
            bboxTile.min = MapPos(bboxTile2D.min(0), -bboxTile2D.min(1));
            bboxTile.max = MapPos(bboxTile2D.max(0), -bboxTile2D.max(1));
        ...
        std::shared_ptr <MapQueryResult> MapFile::readMapData(const MapTile &tile, Selector selector) {
            ...
            MapBounds projectedMapBounds3D = _tileTransformer->calculateTileBBox(flippedTile);
            MapBounds projectedMapBounds2D = Converter::bbox3tobbox2(projectedMapBounds3D);
