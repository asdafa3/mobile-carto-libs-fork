...
namespace carto {
    namespace mapsforge {
        ...
        MapFile::MapFile(const std::string &path_to_map_file, const std::shared_ptr <std::vector<Tag>> &tagFilter, std::shared_ptr<mvt::Logger> logger) :
                ...
                _map_file_header(std::move(logger)),  //TODO: Check move semantics
                _logger(std::move(logger)) {
            try {
                std::shared_ptr<ReadBuffer> readBuffer = std::make_shared<ReadBuffer>(path_to_map_file, _logger);
        ...
        bool MapFile::processWays(const QueryParameters &queryParams, uint32_t numberOfWays, const MapBounds2D &mapBounds,
